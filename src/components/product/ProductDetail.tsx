...
const ProductDetail = () => {
  ...
  const reviewCount = useMemo(() => (product ? 20 + product.id * 5 : 0), [product]);
  ...
  if (isError || !product) {
    return (
      <div className="rounded-lg border border-danger/40 bg-danger/10 p-8 text-center">
        <p className="text-base font-semibold text-danger">{t("product.errorDetails")}</p>
  ...
  const handleAddToCart = () => {
    ...
    pushToast({
      title: t("product.addedToCartTitle", { count: safeQuantity, name: localizedName }),
  ...
  return (
    <div className="space-y-6">
      <section className="grid gap-5 rounded-lg border border-border bg-surface-1 p-5 shadow-soft lg:grid-cols-[1.08fr_1fr] lg:gap-7 lg:p-6">
        ...
        <div className="space-y-4">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Badge variant="neutral">{t("product.skuLabel", { id: product.id })}</Badge>
          </div>

          <div className="space-y-2">
            <h2 className="text-2xl font-bold text-foreground sm:text-3xl">{localizedName}</h2>
          ...
          <p className="text-sm leading-7 text-muted-foreground">{localizedDescription}</p>
          ...
          <dl className="grid gap-2 rounded-md border border-border bg-surface-2 p-3 text-sm">
            ...
            <div className="flex items-center justify-between gap-4">
              ...
              <dd className="font-semibold text-foreground">
                {product.in_stock ? t("product.readyToShip") : t("product.backorder")}
              </dd>
            </div>
          </dl>
        </div>
      ...
      <Tabs defaultValue="description">
        <TabsList>
          <TabsTrigger value="description">{t("product.tabDescription")}</TabsTrigger>
          <TabsTrigger value="specs">{t("product.tabSpecs")}</TabsTrigger>
          <TabsTrigger value="shipping">{t("product.tabShippingReturns")}</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="specs">
          <ul className="grid gap-2 text-sm text-muted-foreground">
            ...
            <li>{t("product.spec2", { sku: `RM-${product.id.toString().padStart(5, "0")}` })}</li>
