...
const ProductGrid = ({
  ...
  const resolvedEmptyMessage =
    emptyMessage || t("shop.emptyProducts");

  if (isLoading) {
    return (
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {Array.from({ length: 8 }).map((_, index) => (
          <div className="rounded-lg border border-border bg-surface-1 p-3" key={index}>
  ...
  if (!products.length) {
    return (
      <div className="rounded-lg border border-border bg-surface-1 p-8 text-center shadow-soft">
        ...
        <p className="text-base font-semibold text-foreground">{t("shop.noProductsFound")}</p>
        <p className="mt-1 text-sm text-muted-foreground">{resolvedEmptyMessage}</p>
        {onResetFilters ? (
          <Button className="mt-4" onClick={onResetFilters} size="sm" variant="secondary">
            {t("shop.resetFilters")}
          </Button>
        ) : null}
      </div>
